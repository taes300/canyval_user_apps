...
void CNDH_ProcessGroundCommand(void)
{
    ...
    switch (CommandCode)
    {
		...
		case CNDH_COMMUNICATION_CC:
			OS_printf("CNDH, Communication mode entry\n");
			coms_LogMsg = (coms_log *)cndhMsgPtr;
			CNDH_Save_data(4);
			CFE_SB_SetCmdCode((CFE_SB_MsgPtr_t) &cndh_comsCmd, 5); //send command 1 to coms for beacon
...
void CNDH_Save_data(int number)
{
	   ...
	   case 4:
		   cndh_LocalLog.Ground_time = coms_LogMsg->time;
		   mode_flag.flag = coms_LogMsg->flag;
		   break;
	    }
}
...
void CNDH_ProcessScheduleCommand(void)
{
	switch (mode_flag.flag)
	{
		...
		case 3: // normal(EO) mode
			...
			OS_printf("Early orbit phase: Normal mode 1U entry\n");
		...
		case 4: //commissioning 1 (GPS)
			OS_printf("Early orbit phase: Commissioning 1 (GPS) mode 1U entry\n");
			OS_printf("CNDH, Commissioning 1 (GPS) doney\n");
			mode_flag.flag = 3;
			break;
	}
